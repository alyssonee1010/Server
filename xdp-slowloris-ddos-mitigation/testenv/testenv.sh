...
setup()
{
    ...
    ip netns add "$NS"
    ip link add dev "$NS" type veth peer name enp0s3 netns "$NS"
    ...
    INSIDE_MAC=$(iface_macaddr enp0s3 "$NS")
    ip neigh add "$INSIDE_IP6" lladdr "$INSIDE_MAC" dev "$NS" nud permanent

    set_sysctls enp0s3 "$NS"
    ip -n "$NS" link set dev lo up
    ip -n "$NS" link set dev enp0s3 up
    ip -n "$NS" addr add dev enp0s3 "${INSIDE_IP6}/${IP6_PREFIX_SIZE}"
    ip netns exec "$NS" ethtool -K enp0s3 rxvlan off txvlan off
    # Prevent neighbour queries on the link
    OUTSIDE_MAC=$(iface_macaddr "$NS")
    ip -n "$NS" neigh add "$OUTSIDE_IP6" lladdr "$OUTSIDE_MAC" dev enp0s3 nud permanent
    # Add route for whole test subnet, to make it easier to communicate between
    # namespaces
    ip -n "$NS" route add "${IP6_SUBNET}::/$IP6_FULL_PREFIX_SIZE" via "$OUTSIDE_IP6" dev enp0s3

    if [ "$LEGACY_IP" -eq "1" ]; then
        ip addr add dev "$NS" "${OUTSIDE_IP4}/${IP4_PREFIX_SIZE}"
        ip -n "$NS" addr add dev enp0s3 "${INSIDE_IP4}/${IP4_PREFIX_SIZE}"
        ip neigh add "$INSIDE_IP4" lladdr "$INSIDE_MAC" dev "$NS" nud permanent
        ip -n "$NS" neigh add "$OUTSIDE_IP4" lladdr "$OUTSIDE_MAC" dev enp0s3 nud permanent
        ip -n "$NS" route add "${IP4_SUBNET}/${IP4_FULL_PREFIX_SIZE}" via "$OUTSIDE_IP4" dev enp0s3
        ENABLE_IPV4=1
    else
        ENABLE_IPV4=0
    fi
    ...
    if [ "$USE_VLAN" -eq "1" ]; then
        ...
        for vid in "${VLAN_IDS[@]}"; do
            ...
            ip -n "$NS" link add dev "enp0s3.$vid" link "enp0s3" type vlan id "$vid"
            ip -n "$NS" link set dev "enp0s3.$vid" up
            ip -n "$NS" addr add dev "enp0s3.$vid" "${inside_ip}/${IP6_PREFIX_SIZE}"
            ip -n "$NS" neigh add "$outside_ip" lladdr "$OUTSIDE_MAC" dev "enp0s3.$vid" nud permanent
            set_sysctls "enp0s3/$vid" "$NS"
        done
    else
        ENABLE_VLAN=0
    fi
    ...
    echo ""
    wait_for_dev "$NS" && wait_for_dev enp0s3 "$NS"
...
populate_redirect_map()
{
    local src="$1"
    local dest="$2"
    local src_mac=$(ip netns exec $src cat /sys/class/net/enp0s3/address)
    local dest_mac=$(ip netns exec $dest cat /sys/class/net/enp0s3/address)
